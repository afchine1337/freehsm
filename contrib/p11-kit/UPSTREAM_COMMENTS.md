...
VERIFIED 27 August 2026 against #745's own branch, commit 7fb676d,
p11-kit/rpc-message.c:

  * mechanism_has_no_parameters() begins at line 2425. Lines 2555-2582 list
    CKM_ML_DSA and CKM_SLH_DSA *bare* — not only the _KEY_PAIR_GEN forms —
    together with the whole CKM_HASH_ML_DSA_* and CKM_HASH_SLH_DSA_* family.
    Comment B's claim is about what that branch actually does.
  * Standard CKM_SHA3_* is still absent from that function; only
    CKM_IBM_SHA3_* is present. Comment A's first point still holds.
  * #745's integration tests (test-kryoptic.sh, test-nss-softokn.sh) cover
    ...
    at all — so the silent context drop could not surface there. Comment B
    said "none signs with a context", which was weaker than the truth.

State of #745 at that date: ueno reviewed favourably on 12 March, mingulov
rebased the same day, two CodeQL notices were answered on 24 March, and review
was re-requested from ueno on 8 April. Nothing since; at least one approving
review is required to merge. Comment A therefore no longer offers to wait for
#745 to land before ours can move — the offer stands, but it is not a
condition. Do not remark on the delay in public: the maintainers are
volunteers, and pointing at a stalled review reads as pressure.
-->
...
1. **Standard `CKM_SHA3_*`.** #745 adds the PQC families; the SHA-3 digests and
   HMACs, in PKCS#11 since 3.0, are still absent from
   `mechanism_has_no_parameters()` on that branch — while `CKM_IBM_SHA3_*` is
   present.
...
I have also left a comment on #745 about `CKM_ML_DSA`, `CKM_SLH_DSA` and the
`CKM_HASH_*` family, which I believe should not be on the no-parameter list.
That is a question about that PR rather than a claim about this one.

On ordering, whichever suits you: #745 is the bigger piece of work and I am
glad to rebase this on top of it, reduced to parts 1, 2 and 3, once it lands.
Equally, parts 1 to 3 here stand on their own and do not depend on it, so if it
is easier to take the smaller change first I can rebase the other way round.
I have no preference beyond wanting the ML-DSA context question resolved before
either lands.
...
On this branch (7fb676d), `mechanism_has_no_parameters()` lists `CKM_ML_DSA`
and `CKM_SLH_DSA` themselves, not only their `_KEY_PAIR_GEN` forms, together
with the whole `CKM_HASH_ML_DSA_*` / `CKM_HASH_SLH_DSA_*` family. Those take an
optional `CK_SIGN_ADDITIONAL_CONTEXT` — `CK_HASH_SIGN_ADDITIONAL_CONTEXT` for
the `CKM_HASH_*` ones (PKCS#11 3.2, §6.18 and §6.19):
...
With those mechanisms on that list, `p11_rpc_buffer_add_mechanism()` writes
nothing after the mechanism type and `p11_rpc_buffer_get_mechanism()` returns
before reading a parameter. So a caller that passes a context — or a
non-default `hedgeVariant` — has it dropped, and the token signs with an empty
context and the default hedging instead. The call succeeds and returns a
signature that verifies against something other than what the caller asked for.

That seems worse than the failure it replaces: today the call is refused, which
is visible.

**The tests would not show it.** `test-kryoptic.sh` and `test-nss-softokn.sh`
cover generate, export, import roundtrip and RPC forwarding — none of them
signs at all, with or without a context. So this would land green.
