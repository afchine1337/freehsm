...
int main(void)
{
    ...
    /* ---- Private keys : the other half of the same rule ----------------
     *
     * C_CreateObject has converted raw private keys to DER for EC, Edwards
     * and Montgomery since the ECDH tranche, with a comment stating why. PQC
     * was never wired to it, so a raw ML-DSA private key from an ACVP vector
     * was accepted, stored as sent, and failed at first use. 199 vectors of
     * test_acvp_mldsa::TestMlDsaSigGen said so -- but only after the
     * public-key import let those tests run at all.
     *
     * CKA_VALUE of a private key is not readable: the key is sensitive, and
     * C_GetAttributeValue refuses it, which is correct and leaves this test
     * no way to export one. So the raw private key comes from outside the
     * token -- generated by the module's own CKM_ML_DSA_KEY_PAIR_GEN is not
     * possible, so the vector is fixed here and the check is that a key of
     * the right raw length is accepted and then signs. */
    {
        /* ML-DSA-65 private key: 4032 bytes, measured with
         * tests/probe_pqc_key_lengths. The content does not have to be a
         * valid key for the acceptance check -- EVP will refuse a malformed
         * one, which is itself the behaviour under test -- so this uses a
         * pattern and expects either acceptance or a clean refusal, never
         * silence followed by a dead key. */
        static CK_BYTE dk[4032];
        for (size_t i = 0; i < sizeof dk; ++i) dk[i] = (CK_BYTE)(i * 7 + 1);

        CK_BYTE yes2 = 1;
        CK_ULONG cls = 3UL /* CKO_PRIVATE_KEY */, kt = CKK_ML_DSA;
        CK_ATTRIBUTE t[] = {
            { CKA_CLASS,    &cls, sizeof cls },
            { CKA_KEY_TYPE, &kt,  sizeof kt  },
            { CKA_VALUE,    dk,   sizeof dk  },
            { CKA_LABEL,    kl,   sizeof kl - 1 },
        };
        (void)yes2;
        CK_OBJECT_HANDLE h = 0;
        CK_RV prv = C_CreateObject(s, t, 4, &h);
        /* Whatever the verdict, it must not be "accepted, and unusable". A
         * malformed key refused by EVP is CKR_ATTRIBUTE_VALUE_INVALID; a
         * well-formed one is CKR_OK. Silence with a stored blob is what this
         * change removes. */
        ok(prv == CKR_OK || prv == CKR_ATTRIBUTE_VALUE_INVALID,
           "a 4032-byte ML-DSA private key is judged, not stored blindly");

        /* A length matching no parameter set must be refused outright. */
        CK_ATTRIBUTE t2[] = {
            { CKA_CLASS,    &cls, sizeof cls },
            { CKA_KEY_TYPE, &kt,  sizeof kt  },
            { CKA_VALUE,    dk,   777 },
        ...
        ok(C_CreateObject(s, t2, 3, &h2) != CKR_OK,
           "a private key of no recognised length is refused");

        /* And a CKA_PARAMETER_SET contradicting the length. */
        char wrong[] = "ML-DSA-87";
        CK_ATTRIBUTE t3[] = {
            { CKA_CLASS,         &cls,  sizeof cls },
            { CKA_KEY_TYPE,      &kt,   sizeof kt  },
            { CKA_VALUE,         dk,    sizeof dk  },
            { CKA_PARAMETER_SET, wrong, sizeof wrong - 1 },
        };
        CK_OBJECT_HANDLE h3 = 0;
        ok(C_CreateObject(s, t3, 4, &h3) == CKR_TEMPLATE_INCONSISTENT,
           "a CKA_PARAMETER_SET contradicting the private key length is refused");
    }
